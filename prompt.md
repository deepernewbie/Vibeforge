...
Easy to forget and the failure is confusing — the build dies at
`checkReleaseAarMetadata` and prints every AndroidX artifact it found, which
reads like a dependency problem rather than a missing one-line setting.
...
Any project with an `androidx.*` dependency needs this file. That is nearly
all of them.
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Collect APK
        run: |
          echo "--- everything under app/build/outputs ---"
          find app/build/outputs -type f -name '*.apk' -printf '%p  %s bytes\n' || true
          SRC=$(find app/build/outputs -type f -name '*.apk' | head -1)
          if [ -z "$SRC" ]; then
            echo "::error::assembleRelease finished but produced no APK."
            exit 1
          fi
...
### The theme must match the Activity class

`AppCompatActivity` requires a `Theme.AppCompat` descendant. Pairing it with a
platform theme — `@android:style/Theme.Material.Light` and friends — throws
`IllegalStateException: You need to use a Theme.AppCompat theme (or descendant)
with this activity` the instant the app opens. It is the most common first-run
crash, and from the phone it looks like the app simply closes.

Pick one pair and keep it:

- `android.app.Activity` + `@android:style/Theme.Material.Light.NoActionBar`,
  no appcompat dependency, no `res/values/themes.xml`. Prefer this.
- `AppCompatActivity` + `@style/Theme.AppCompat.Light.NoActionBar`, with the
  appcompat dependency present.

Never `AppCompatActivity` with a platform theme.

### Record your own crashes

There is no logcat on the other end of this. Install an uncaught-exception
handler in `onCreate` that writes the stack trace to `filesDir`, and display
it on the next launch. Fifteen lines, and it turns "it just closes" into a
stack trace you can read.
...
### Keep slow work off onCreate

`EncryptedSharedPreferences`, file scans and network calls all belong on a
background thread. On the main thread during `onCreate` they stall startup and,
when the keystore misbehaves, crash before anything is drawn.
...
| `useAndroidX property is not enabled` | no `gradle.properties`, or it is missing `android.useAndroidX=true` |
...
The most confusing outcome, because nothing looks wrong. Usual causes:

- **Workflow permissions.** Repository → Settings → Actions → General →
  Workflow permissions must be *Read and write*. On the read-only default the
  release step is refused and, without `fail_on_unmatched_files`, says nothing.
- **`find` looked in the wrong place.** Search all of `app/build/outputs`
  rather than one subdirectory; flavours and split builds move it.
- **The release exists but was missed.** With `tag_name: latest` it is a
  prerelease, listed under Releases rather than on the repository front page.

Always upload an artifact as well. It costs one step and means a successful
build is never wasted.
